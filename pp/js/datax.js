...
function createBoxHTML(item) {
    ...
    if (item.item.status === 'live') {
        ...
        if (boxcontent === "nocontent") {
            boxcontent = ""; // if it's nocontent, we want to display an empty string
        }
        ...
        if (status === "live" && h1 !== "") {
            boxHTML = `<li class='box illuminate ${item.item.colour} inactive ${randomTransition}' id='box-${item.id}' tabindex='${item.id}'>
